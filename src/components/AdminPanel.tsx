      ...
      <div className="bg-gray-50 rounded-xl p-4 space-y-3">
          ...
          <div key={v.id} className="bg-white rounded-lg px-3 py-2.5 text-xs space-y-1.5">
            <div className="flex items-center justify-between">
              <div className="flex gap-1 flex-wrap items-center">
                ...
                {parseFloat(v.price) > 0 && <span className="font-medium text-green-700">${parseFloat(v.price).toFixed(2)}</span>}
                {parseFloat(v.price) <= 0 && <span className="text-gray-400 text-[10px]">Usa precio del producto</span>}
              </div>
        ...
        <form onSubmit={handleAddVariant} className="space-y-2">
          ...
          <details className="text-xs">
            ...
            <input type="number" step="0.01" value={varPrice} onChange={(e) => setVarPrice(e.target.value)} placeholder="Dejar en 0 para usar el precio del producto" className="w-full px-2 py-1.5 border rounded text-sm mt-1" />
          </details>
        </form>
